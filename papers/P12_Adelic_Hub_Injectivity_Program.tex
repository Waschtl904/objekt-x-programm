...
\(R>0\).  Below the global \(\rho\)-threshold an additional exact
\(42\times42\) raw-operator certificate closes one explicit nonempty
overlap cell.
...
A15.1b2g cell already closed locally.
...
\]
Below \(\rho\), Proposition~\ref{prop:p12-r23-c23} gives an additional
local exact kill in the explicit overlap cell (C23).
...
\begin{remark}[Audit status after Round 23]
The full \(\rho\)-descent is `\(\checkmark[M]\)`.  The all-radius
restricted-tail theorem is `\(\checkmark[M]\)`.  The exact Round-23 overlap
seed is `\(\checkmark[M]_{\rm part}\)`: it is a local theorem only and does
not imply a global descent to \(\omega\).
\end{remark}
...
\ker L_{R,S,T_0}^{\{a,b,2a\}}=\{0\}.
\]
The explicit C23 cell of Proposition~\ref{prop:p12-r23-c23} is already
closed locally, but no full theorem of the form \(R\ge\omega\) is currently
booked.
\end{openproblem}
