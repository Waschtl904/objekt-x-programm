...
NEU-091--120 einschließlich NEU-120 Patch 2 und der P10-Reconciliation vom
9. August 2026. Es ist ein internes SYN-Manuskript; eine Behauptung der
...
\begin{theorem}[Determinanten-No-Go, reconciliert]\label{thm:det-no-go}
...
\]
Der Grenzwert ist $z$-unabhängig und nullstellenfrei. Folglich entsteht aus
dieser konkreten Skalierung kein direkter nichttrivialer $C\xi(z)$-Anschluss
über den Determinantenweg.
\end{theorem}

\begin{remark}[Status und Scope]
Die historische NEU-091-Aussage
\[
  D_N(z)\to e^{-\gamma^2/4}
\]
ist durch P06 G-T4/G-T5 und den P10-Targeted-Reaudit
\texttt{f0be54c5} \emph{SUPERSEDED}. Der korrigierte Befund
$D_N(z)\to1$ hat Status $\checkmark[M]_{\rm neg}$ im angegebenen
Modellscope. Daraus folgt ausdrücklich kein universeller Feshbach-, Fredholm-
oder Determinanten-No-Go; andere Skalierungen, Renormierungen und
$\det_2$/Weil-Hilbertisierungen bleiben offen.
\end{remark}
...
\begin{remark}[Externer GM-Gegencheck]\label{rem:external-gm}
Ein externer Gegencheck beanstandete eine vermeintliche Spezialisierung
$H=M$. Diese steht im Live-Manuskript nicht; der selbstduale Punkt ist
$H=\sqrt M$. Der erneute Quellenabgleich bestätigte außerdem den auf der
Kurzintervallseite verwendeten Bereich $1\le H\le M^{1-\varepsilon}$.
Die Untergrenze $M^\varepsilon$ gehört in der klassischen Äquivalenz zur
korrespondierenden Pair-Correlation-$T$-Seite und wird hier nicht als
zusätzliche $H$-Untergrenze importiert.
\end{remark}
...
Die Umkehrung
\[
  \text{Rampenform}\Longrightarrow\LFF
\]
ist nicht bewiesen und nicht widerlegt; sie bleibt offen.
\end{theorem}
...
\]
geschrieben. Für die kanonische Zentrierung
\[
  \Xi(z)=\xi\!\left(\frac12+iz\right),
  \qquad m_{\rm arith}(z)=-\frac{\Xi'(z)}{\Xi(z)},
\]
folgt aus $\Xi(-z)=\Xi(z)$ und der reellen Symmetrie
\[
  \boxed{A=\Re m_{\rm arith}(i)=0.}
\]
Die äquivalente Nullstellensumme ist im kanonisch symmetrischen Sinn zu
verstehen.
\end{definition}
...
\begin{theorem}[Notwendiger Symmetrie-/Normalisierungstest]
\label{thm:aN-zero}
Bei $z=i$ gilt
\[
...
\]
Da $\Re m_{\rm arith}(i)=0$, folgt notwendig
\[
  \boxed{
  \widetilde m_N^{\rm ren}
  \xrightarrow[N\to\infty]{\rm loc.\,unif.}m_{\rm arith}
  \quad\Longrightarrow\quad a_N\to0.}
\]
Dies ist keine zusätzliche RH-Annahme, sondern eine notwendige Konsequenz der
bereits offenen lokalen gleichmäßigen Konvergenz.
\end{theorem}
...
\begin{enumerate}
  ...
  \item eine kanonische Skalierung $c_N>0$ und Kontrolle der Konstanten
        $a_N\in\mathbb R$, wobei jede erfolgreiche Folge nach
        Theorem~\ref{thm:aN-zero} notwendig $a_N\to0$ erfüllen muss;
...
\midrule
\endhead
$D_N(z)\to1$ im NEU-088--90-Scaling; kein nichttrivialer $C\xi(z)$-Grenzwert aus dieser Skalierung
& PROVED$_{\rm neg}$ (modell-/skalenspezifisch) & P06 G-T4/G-T5; P10\\
$D_N(z)\to e^{-\gamma^2/4}$ im selben Scaling
& SUPERSEDED & NEU-091; P10\\
...
& CONDITIONAL (RH+SPC) & NEU-101\\
Externer $H=M$-Gegenbefund gegen das aktuelle Korollar
& NO-GO; trifft Live-Stand nicht & Targeted Reaudit\\
...
Rampenform $\Rightarrow\LFF$
& OPEN; nicht bewiesen/widerlegt & NEU-107; P10\\
...
& OPEN & NEU-120\\
Lokale glm. Konvergenz $\widetilde m_N^{\rm ren}\to m_{\rm arith}$
$\Rightarrow a_N\to0$
& PROVED (notwendige Bedingung) & NEU-120.3\\
...
\emph{SYN FINAL AUDITED / P10-RECONCILED}, Patch 5 vom 9. August 2026.
Die Goldston--Montgomery-Synchronisierung von NEU-101 liegt in Commit
\texttt{92d731d1}; die Symmetriepräzisierung $A=0$ und der notwendige Test
$a_N\to0$ liegen in NEU-120 Patch 2, Commit \texttt{410d0a91}, und im
Targeted-Reaudit \texttt{57441d87}. Die Determinanten-Reconciliation liegt im
P10-Targeted-Reaudit \texttt{f0be54c5}; die LFF-Rückstufung auf OPEN für die
Umkehrung ist durch den P10-Gegencheck \texttt{c77a101} gebunden. P02 liefert
die hier referenzierten kanonischen Fourier-, Paley--Wiener- und
Weilform-Konventionen. P03 wurde nur für SYN-Stil und
Konventionskonsistenz herangezogen; es werden daraus keine zusätzlichen
P07-Aussagen importiert.
\end{remark}

\begin{remark}[LaTeX-SYN-Finalaudit]\label{rem:audit-status}
Der ursprüngliche LaTeX-Transferaudit ist abgeschlossen. Patch 5 ist eine eng
begrenzte P10-Reconciliation: Er ersetzt ausschließlich den durch P06 G-T4/G-T5
supersedierten Determinantengrenzwert und korrigiert die logisch zu starke
Biimplikationsklassifikation im LFF/Rampenkanal. Kein historischer
Forschungsknoten wird dadurch zu einer RH-unabhängigen Konvergenzaussage
hochgestuft. Damit bleibt der interne Status
\[
