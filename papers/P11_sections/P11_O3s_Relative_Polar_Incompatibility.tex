...
noncommuting'' is insufficient as stated.  The remaining P11 problem is a relative
polar-compatibility problem: one must detect a failure of the source and target polar
rotations to intertwine under $W$.

Equivalently, the genuinely decisive quantity remains
\[
 \mathfrak P_U=U_SW-WU_R,
\]
or, by Proposition~\ref{prop:o3n-gauge-criterion}, the true gauge compression
$\Gamma_U-I$.  Any useful commutator criterion must therefore be relative across the
nested metric pairs, not a pair of separate estimates on
