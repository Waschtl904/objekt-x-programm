...
absolute bound to the discrete primitive certificate.  The trace-aware
source-representer estimate~\eqref{eq:odd-quadrature-trace-bound}, with uniform
smooth source bounds and $K_U=O(\sqrt U)$, gives
\[
 \|Z_U^{\rm quad}\|_2
 \le C e^{-3U/10}+C\frac{|K_U|}{U}e^{-2U/5}=o(1).
\]
Both moving endpoint traces of the zero-extended mean subtraction are
included; no global Hilbert-Lipschitz claim is used.  Finally
