...
\]
and, since $C_T^-(r,t)=2k_T^0(t)\alpha(2r-t)$,
\[
 \boxed{\partial_r\Phi_T(r)(v)
 =4k_T^0(v)\alpha'(2r-v)-4(k_T^0)'(2r-v)\alpha(v).}
\tag{6.21c}\label{eq:odd-source-representer-derivative}
\]
Hence~\eqref{eq:odd-kernel-bound} controls the required Hilbert-space Lipschitz norm;
no operator-norm continuity of the raw reflection is used.  Apply
\eqref{eq:prime-cell-quadrature} to $\Phi_T$.  The cost
