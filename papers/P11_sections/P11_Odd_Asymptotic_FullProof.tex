...
Set $\mathscr H_T:=L^2(-T,T)$ and put
\[
 h_T:=H_T^*J_{R,T}f_-,
 \qquad
 A_T:=I+R_T^*R_T.
...
\paragraph{Step 1: matching lower scale and the sharp constant-mode denominator.}
...
\]
We next prove the sharp denominator
\[
 \boxed{
...
\]
The point is the exact martingale suppression, not absolute summability of the raw rest
coefficients.  For fixed $p$ let $J=J_{p,T}(u)$.  If $J=0$, then
$\mathsf Q_T(u)\eta_{p,k}=0$ for every $k$.  If $k\le J$, both translated points remain
inside $(-T,T)$ and hence
$D_{k\log p}E_T\mathbf1_T(u)=0$.  Thus only $1\le J=j<k$ contributes.  From
\eqref{eq:eta-pk}--\eqref{eq:source-cutoff}, orthonormality of the martingale basis gives
\[
 \|\mathsf Q_T(u)\eta_{p,k}\|^2
 =p^{j-k}-p^{-k}\le p^{j-k}.
\tag{6.4a}\label{eq:constant-mark-suppression}
\]
Let $F_{p,T}$ denote the $p$-sector of $R_T\mathbf1_T$.  On the shell
$J_{p,T}(u)=j\ge1$,
\[
 \|F_{p,T}(u)\|
 \le
 \frac{\sqrt{\log p}}{1-p^{-3/4}}\,p^{-j/4-3/4}.
\tag{6.4b}\label{eq:constant-p-shell-pointwise}
\]
Each such shell has measure at most $\log p$.  Summing the geometric series in $j$ gives
\[
 \|F_{p,T}\|_2^2
 \le
 \frac{(\log p)^2}{(1-p^{-3/4})^2(1-p^{-1/2})}\,p^{-2}.
\tag{6.4c}\label{eq:constant-p-shell-L2}
\]
Different prime sectors are orthogonal.  Therefore
\[
 \|R_T\mathbf1_T\|_2^2
 =\sum_p\|F_{p,T}\|_2^2
 \le C\sum_p\frac{(\log p)^2}{p^2}<\infty
\tag{6.4d}\label{eq:constant-rest-uniform}
\]
uniformly in $T$.  Since $\|\mathbf1_T\|_2^2=2T$, equation
\eqref{eq:odd-constant-sharp} follows.
...
\[
 ...
 =\int_0^Tk_T(t)\overline{b_T(t)}\,dt,
...
\[
 \int_0^Tk_T^0(t)\overline{b(t)}dt
 =\int_0^T\int_0^\varepsilon
 k_T^0(t)\alpha(s)
 \bigl[\overline{b(t)}-\overline{b(s)}\bigr]\,ds\,dt.
...
\]
This is a consequence of Guth--Maynard~\cite{GuthMaynard2026}; we take
$\theta=3/5$.  Prime powers with exponent at least two contribute
$O(x^{1/2}\log x)=o(x^{3/5})$, so the same scale supplies the required prime mass.
...
\]
The field used here is the source representer, not the raw reflection operator.  Extend
$C_T^-(r,\cdot)$ by zero outside its natural support and define on $L^2(0,T)$
\[
...
\]
The zero extension of $k_T^0$ generally has jumps at $0$ and $T$.
In particular $k_T(T)=0$ by the fixed support gap, whereas
$k_T^0(T-)=-K_T/T$.  Thus the following derivative is only the
regular part, away from the moving jump lines:
\[
 \boxed{\partial_r\Phi_{T,\mathrm{reg}}(r)(v)
 =4k_T^0(v)\alpha'(2r-v)-4(k_T^0)'(2r-v)\alpha(v).}
\tag{6.21c}\label{eq:odd-source-representer-derivative}
\]
No global Hilbert-space Lipschitz assertion is made for $\Phi_T$.
Instead put $L_T=(T+\varepsilon)/2$ and form the signed quadrature measure
\[
 d\mu_T=\sum_{I,q:r_q\in I}\lambda_q^{(I)}\delta_{r_q}
       -1_{[0,L_T]}(r)\,dr,\qquad
 F_T(r)=\mu_T([0,r]).
\]
The positive weights and exact cell masses give, with the usual one-sided
endpoint convention,
\[
 F_T=0\ \text{at cell boundaries},\qquad
 |F_T(r)|\le |I|\le C e^{-\frac45(T-r)}\quad(r\in I).
\tag{6.21d}\label{eq:odd-quadrature-cumulative}
\]
For almost every fixed $v$, scalar integration by parts includes both jumps
of $r\mapsto\Phi_T(r)(v)$: they are
$-2k_T^0(0+)\alpha(v)$ at $r=v/2$ and
$2k_T^0(T-)\alpha(v)$ at $r=(T+v)/2$.
Coincidences with the finitely many quadrature atoms form a null set of $v$.
Consequently, after taking the $L^2_v$ norm and using Minkowski,
\[
 \begin{aligned}
 \left\|\int\Phi_T(r)\,d\mu_T(r)\right\|_2
 \le{}&
 \int_0^{L_T}|F_T(r)|\,
       \|\partial_r\Phi_{T,\mathrm{reg}}(r)\|_2\,dr\\
 &+2|k_T^0(0+)|\,\|\alpha(v)F_T(v/2)\|_2\\
 &+2|k_T^0(T-)|\,\|\alpha(v)F_T((T+v)/2)\|_2.
 \end{aligned}
\tag{6.21e}\label{eq:odd-quadrature-with-traces}
\]
This is pointwise scalar bounded-variation integration, not an assertion
of Hilbert-valued bounded variation.

On $0\le r\le T/2$, the kernel bound~\eqref{eq:odd-kernel-bound},
up to fixed $\varepsilon$-shifts, gives
\[
 \|\partial_r\Phi_{T,\mathrm{reg}}(r)\|_2
 \le C_f\frac{e^{(T-2r)/2}}{\sqrt{1+T-2r}}
       +C_\alpha\frac{|K_T|}{T}.
\]
On the remaining fixed-width strip the growing kernel vanishes by
$a_*>\rho_f+2\varepsilon$, and only the second bound is needed.
The growing part of the weighted integral is bounded by
$C_f e^{-3T/10}\int_0^{T/2}e^{-r/5}\,dr$.
The constant part is bounded by $C_\alpha |K_T|T^{-1}e^{-2T/5}$.
At the two jumps, respectively,
\[
 |F_T(v/2)|\le C e^{-4T/5},\qquad
 |F_T((T+v)/2)|\le C e^{-2T/5}
 \quad(v\in\operatorname{supp}\alpha),
\]
and
$|k_T^0(0+)|\le C_f e^{T/2}/\sqrt T+|K_T|/T$,
$|k_T^0(T-)|=|K_T|/T$.
Thus the source quadrature remainder satisfies the explicit bound
\[
 \boxed{\|Z_T^{\rm quad}\|_2
 \le C_f e^{-3T/10}
      +C_\alpha\frac{|K_T|}{T}e^{-2T/5}.}
\tag{6.21f}\label{eq:odd-quadrature-trace-bound}
\]
The fixed even-source identification only changes the harmless bound constant.

The certificate costs involve $C_T^-(r,\cdot)$ itself and do not require a
Lipschitz claim for its reflected representer.  Its direct local envelope,
together with~\eqref{eq:prime-cell-cost}, gives
\[
 \|Y_T^{\rm prim,-}\|^2
 \le C_f\left(T^{-1}+Te^{-T/4}
       +\frac{|K_T|^2}{T^2}e^{-T/2}\right).
\]
This produces the same primitive
discrete certificate $Y_T^{\rm prim,-}$ with
\[
 \boxed{\|Y_T^{\rm prim,-}\|^2=o(M_T).}
\tag{6.22}\label{eq:odd-prim-cost}
\]
Equation~\eqref{eq:odd-quadrature-trace-bound} and
$M_T=|K_T|^2/(2T)$ give
\[
 \boxed{\|Z_T^{\rm quad}\|_2=o(\sqrt{M_T}).}
\tag{6.23}\label{eq:odd-quadrature-error}
\]
This also preserves the absolute near-null quadrature conclusion when
$K_T=O(\sqrt T)$ and the smooth source bounds are uniform in the relevant
fixed finite family.  In that case the displayed error is $o(1)$.
Both endpoint traces, including that of the mean-zero constant subtraction,
have been retained.
