...
\tag{DT.2}\label{eq:jet-beta}
\]
Put
\[
 c_m:=\frac{\binom{2m}{m}}{4^m}.
\tag{DT.3}\label{eq:jet-cm}
\]
For $T>R$ let
\[
 \mathbf1_T:=1_{(-T,T)},
...
\tag{DT.8}\label{eq:PhiT-asymptotic}
\]
The binomial expansion
\[
 (1-z)^{-1/2}=\sum_{m=0}^{M}c_mz^m+O_M(z^{M+1})
\]
is uniform for $0\le s\le R$ and large $T$.  Substitution into~\eqref{eq:PhiT-asymptotic}, followed
by integration against $-\operatorname{sgn}(u)f(u)$, proves~\eqref{eq:jet-expansion}.  Equation
\eqref{eq:jet-pullback} is immediate because zero extension does not change the integral over the
old support.
\end{proof}
...
\tag{DT.13}\label{eq:absolute-terminal-lower}
\]
Consequently
\[
...
$|\ell_T(f)|\asymp_f e^{T/2}T^{-1/2}$.  Equations~\eqref{eq:sigma-constant-lower}--\eqref{eq:constant-coarse} imply
\eqref{eq:absolute-terminal-lower}.  Since
\[
...
\tag{DT.14}\label{eq:parity-intertwining}
\]
...
\tag{DT.15}\label{eq:parity-split}
\]
...
\tag{DT.16}\label{eq:W-parity-split}
\]
\end{proposition}

\begin{proof}
The Gamma multiplier is even, so the Gamma form is reflection invariant.  Equation
\eqref{eq:parity-intertwining} and commutation of $B_R$ with reflection give the same invariance for the
Schur form.  Thus reflection is unitary in the $X$-graph scalar product, proving
\eqref{eq:parity-split}.  Zero extension intertwines reflection; taking adjoints and continuous
functional calculus then gives the remaining assertions.
\end{proof}

Every kernel in~\eqref{eq:jet-beta} is odd.  Hence $\beta_R^{(m)}$ vanishes on the even sector,
while for odd $f$
\[
 \beta_R^{(m)}(f)=2\int_0^R I_m(r)f(r)\,dr.
\tag{DT.17}\label{eq:beta-odd}
\]
...
\tag{DT.18}\label{eq:jet-kernel}
\]
\end{theorem}

\begin{proof}
For $g=f|_{(0,R)}$, Fubini and~\eqref{eq:beta-odd} give for every $m$
\[
...
norm, the decomposition~\eqref{eq:parity-split} gives~\eqref{eq:jet-kernel}.
\end{proof}
...
\tag{DT.21}\label{eq:cross-kernel-def}
\]
Then
\[
 \|K_{R,S}^{T,U}\|\le1
...
\tag{DT.22}\label{eq:cross-kernel-formula}
\]
...
\tag{DT.23}\label{eq:cross-cauchy}
\]
Thus strong Cauchy convergence is exactly a cross-terminal statement about the
Hermitian real part of~\eqref{eq:cross-kernel-def}, not about convergence of the absolute metrics.
\end{theorem}
...
\tag{DT.25}\label{eq:gamma-dilation}
\]
...
Therefore $C_c^\infty(-R,R)$ is a Gamma-form core.  Proposition~\ref{prop:bounded-schur}
shows that the Gamma form norm and the $X$-graph norm are equivalent at fixed $R$, so
$C_c^\infty(-R,R)$ is also an $X$-graph core.

The odd projection $(I-\mathsf P_R)/2$ is bounded on the graph space by
Proposition~\ref{prop:parity}; applying it to a smooth core sequence proves~\eqref{eq:odd-core}.
...
core and to the exact kernel~\eqref{eq:cross-kernel-formula}.  The remaining difficulty is the asymptotic
