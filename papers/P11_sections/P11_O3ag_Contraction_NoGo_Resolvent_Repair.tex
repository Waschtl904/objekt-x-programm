...
% and unconditional rescaled Neumann-series repair of an earlier audit route.
% Records the two-window/two-prime projection argument showing \|R_{T_0}\|>1 for
% every T_0>log 3, replaces the earlier imprecise divergent-tail language by an
...
\subsection{Contraction no-go for \(R_{T_0}\) and repair of the earlier Neumann route}
\label{subsec:o3ag-contraction-nogo}

An earlier audit route proposed transferring regularity to
\(g_{R,S}=-B_{T_0}H_{T_0}E_{S,T_0}j_{R,S}\) via
\(B_{T_0}=(I+R_{T_0}^*R_{T_0})^{-1}\), under the unscaled contraction hypothesis
\[
 \|R_{T_0}\|<1.
\]
This subsection closes that large-horizon contraction proposal negatively, replaces an
imprecise infinite-tail formulation by an exact finite resolvent remainder, and records
an unconditional rescaled Neumann representation.  It records neither Object~X nor
...
\[
 ...
 =\frac{\log2}{\sqrt2}+\frac{4\log3}{3\sqrt3}\approx1.335841.
 }
\]
Hence
\[
...
\label{cor:o3ag-nogo}
For every \(T_0>\log3\), the unscaled contraction hypothesis
\[
 \|R_{T_0}\|<1
\]
fails.  In particular the full unscaled Neumann series
\(\sum_{n\ge0}(-1)^n(R_{T_0}^*R_{T_0})^n\) diverges in operator norm on this range, so it
cannot be used as an operator-norm identity or as a uniform termwise regularity-transfer
argument for \(B_{T_0}\).  This does not assert that the corresponding vector series
must diverge for every particular \(w\); a special vector may live in a smaller spectral
subspace.
\end{corollary}
...
This is the legitimate replacement for the earlier informal ``tail''
\(\sum_{n>N}(-1)^n(R_{T_0}^*R_{T_0})^nw\).  At
...
\(\|R_{T_0}\|<1\).  Thus the earlier question, read as ``does a Neumann-type
representation of \(B_{T_0}\) exist,'' is answered affirmatively and unconditionally.
The live issue is instead whether the iterates \(A^{N+1}\)
(Lemma~\ref{lem:o3ag-exact-remainder}) or \(Q_M^n\)
(Lemma~\ref{lem:o3ag-rescaled-neumann}) carry regularity or support structure usable for
R32-F(i).  This is a smoothing/fingerprint question about the iterates, not a
spectral-gap question, and is not decided here.
\end{remark}

\begin{remark}[Split status of the earlier Neumann proposal]
\label{rem:o3ag-r34c-split}
The earlier proposal is accordingly split into: (a) the naive unscaled contraction
route \(\|R_{T_0}\|<1\) --- closed negatively by
Theorem~\ref{thm:o3ag-contraction-nogo} for every \(T_0>\log3\); (b) existence of
\emph{some} convergent series for \(B_{T_0}\) --- answered affirmatively and
unconditionally by Lemma~\ref{lem:o3ag-rescaled-neumann}; (c) regularity/fingerprint
structure of the iterated remainder --- genuinely open, and unaffected by (a) or (b).
Only (c) survives as live mathematical content.
\end{remark}
