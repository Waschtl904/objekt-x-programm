...
\qquad l(x) = h(T - x) = 0,
\qquad h(e + x) = 0
\qquad\text{for every } x \in (R, \sigma).
\]
...
\begin{remark}[Scope: what is and is not proven]
...
strictly smaller than the full b2d chamber.  The open frontier is
now
\[
\{\sigma > d/2\} \quad\text{and}\quad \{\sigma > R + \eta\}.
\]
The first is the case-B tail regime (where both $H(x)$ and
...
Full b2d remains Open Problem~\ref{open:p12-subwedgeA-open}.
\end{remark}
