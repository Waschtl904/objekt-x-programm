...
\begin{proof}
Horizon: all six positions are strictly less than $c$ and hence less
than $T_{0}$; in particular $T + x < T + \varepsilon = T_{0}$.
...
to their positive image) or above $c$ (outside horizon, zero) yields
the six equations exactly.  Reflection at zero uses
$h(-y) = -h(y)$, which is the natural convention for an
