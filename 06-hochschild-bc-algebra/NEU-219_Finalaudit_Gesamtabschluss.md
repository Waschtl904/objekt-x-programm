...
**Datum:** 2026-07-25 | **Abschlusskorrektur:** 2026-08-05  
...
| $\lambda^*$ | **existiert nicht** für kanonischen Lift; $\lambda$ ist wirkungslos (wegen $\varepsilon=0$) | NEU-219y2, §10 |
...
**Beweis:** NEU-219y2 §10, §11, §12 (Commit 94f98aa). Zeuge: $a_0^\star = \mu_P^*$, mit
$$
W(\mu_P^*) = -n^{-\beta}\,\omega_{\beta,\chi}(G_P) \neq 0
$$
für alle zulässigen extremalen $\chi$ (strikte Positivität NEU-219y2 §8).
...
|---|---|---|
| `[O-219-5e1j-unit-slot-witness]` | ✓[M] | NEU-219y2 §9, Commit 94f98aa |
| `[O-219-5e1j-explicit-cup-rotation]` | ✓[M]neg | NEU-219y2 §10–12, Commit 94f98aa |
| `[O-219-5e1j-Dg-global-target]` | ✓[M] | NEU-219y1/x, Commit e69e180 |
...
| `[O-219-r3]` | ✓[M]neg — **unbedingt** | NEU-219y2 §12 (ersetzt bedingte Begründung aus NEU-219t) |
| `[O-219-cyclic-representative]` | **?[O]** | Nicht durch Unit-Slot-Zeugen entschieden — exportiert nach §4 |

### Quellfehler-Vermerke (×[M])

Die folgenden Aussagen in der Rollback-Kette sind **×[M]** und müssen in den Quelldateien korrigiert werden; sie beschädigen den Unit-Slot-No-go nicht:

| Quelldatei | Falsche Aussage | Korrekte Ersatzform |
|---|---|---|
| NEU-219x | $D_g(e(r))=0$ | $D_g^{\mathrm{corr}}(e(r))=\mu_m C_{m,n;r}\mu_n^*$ |
| NEU-219x | $D_g(B_{\mathrm{alg}})=0$ (globale Nullaussage) | Mitteltterm $\mu_a D_g^{\mathrm{corr}}(f)\mu_b^*$ fehlt in der Leibnizformel |
| NEU-219x | „gleichmäßiger Grenzwert" von $\mathrm{ad}(Y_N)$ | Nur **punktweise Normkonvergenz** auf jedem festen $a\in A_{\mathrm{alg}}$ |
| NEU-219y1 | Transportformel $G_{nk/\delta,d/\delta}$ | Korrekt: $\sigma_n(G_{k,d})=G_{nk,\,d/\delta}-\rho_{d/\delta}G_{n/\delta,1}$, erster Index $nk$ |
| NEU-219y2 (Einleitung) | $D_g(e(r))=0$ als „bestätigt" | Historisch überholt; Hauptbeweis (Unit-Slot) bleibt vollständig gültig |
...
- NEU-219z (Commit 5415920): $\Phi_0$ und $t\Phi_0$ besitzen verschiedene Determinantenfaktoren ($\Delta_{\mathbf{p}}(h_2,h_3,h_4)$ vs. $\Delta_{\mathbf{p}}(h_1,h_2,h_3)$) und verschiedene $D_g^{\mathrm{corr}}$-Slots.

Die stärkere Aussage $t\Phi_0 \neq C\Phi_0\ \forall C \in \mathbb{C}$ (NEU-219y2) macht eine Neuberechnung von $s$ obsolet.
...
| `[O-219-cyclic-representative]` | `[O-220-2-noncanonical-rotation-repairs]` | Existiert ein zyklischer oder getwistet-zyklischer Ersatzrepräsentant derselben Hochschildklasse? Nicht durch den Unit-Slot-Zeugen ausgeschlossen. Zu klären: Kohomologietyp (gewöhnlich / $\tau$-getwistet / parazyklisch / SAYD). |
...
**Strategischer Hinweis:** Die fehlende Komponente von Objekt $X$ kann nach NEU-219y2 **nicht** bloß ein skalares Orbitgewicht sein. Objekt $X$ muss mindestens einen echten archimedischen bzw. Gamma-Korrekturbaustein, eine nichtskalare Rotationskomponente oder eine wesentlich nichtkanonische Liftstruktur enthalten.
...
| `NEU-219x_Direktaudit_Dg_Primaerformel_und_Fortsetzung.md` | — | Rollback-Stufe 3 + Zieltypfrage; enthält ×[M]-Quellfehler (siehe §3) |
| `NEU-219y_Direktaudit_Dg_Zieltypbruecke_NEU211_NEU217.md` | **NEU-219y1** | Zieltypbrücke; Transportformel korrigiert (siehe §3) |
| `NEU-219y_Unit-Slot-Zeuge_und_Abschluss_des_Basislift-Rotationspfads.md` | **NEU-219y2** | **Hauptabschluss**, Commit 94f98aa; Einleitung historisch überholt (siehe §3) |
| `NEU-219z_Expliziter_Cup-Rotationsaudit.md` | — | Rahmen (Vorgänger von NEU-219y2) |
