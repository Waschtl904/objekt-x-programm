...
## Kanonische Definitionen (alle in P02, §1–§5)

> Fourier-Konvention, $P_{\text{Haar}}$, $R_{\text{PW}}$, $C_{a,b}$, $g_{a,b}$, $h_{a,b}$, $\gamma_\infty$, $\Lambda_\Gamma$, $B_W$ — **alle kanonisch in P02 definiert**. Andere Papers verweisen auf P02 und geben höchstens Erinnerungen.
...
| NEU-250n–r | P02 | §2–§3 | Port-Karte, $R_{\text{PW}}$, $g_{a,b}$ | `INCORPORATED` ✓[K/M] |
| NEU-252 (Patch) | P02 | §6 | Hermitesche Weilform | `INCORPORATED` ✓[K/M] |
| NEU-255 (Patch 2) | P03 | §2 | $H_0=L^2$, Koisometrie | `INCORPORATED` ✓[K/M] |
| NEU-256 (Patch) | P03 | §3 | $R_{\text{arith}}$; KLMN $\times[M]$ | `INCORPORATED` ✓[K/M] |
| NEU-257 (Patch 3) | P03 | §3 | Closability-Firewall | `INCORPORATED` ✓[K/M] |
| NEU-258 (Patch 1) | P02 | §7 | Literaturidentifikation $W_{\text{NEU-252}}=W_{\text{Lit}}$ | `INCORPORATED` ✓[K/M] |
| NEU-259 | P03/P12? | — | RH-freie $\mathcal{H}_W$-Konstruktion | `OPEN` ?[O] |

---

## Offene Migrationspakete (nach P02/P03 Re-Audit)
...
| P02 | Adelic Weil Amplitude Port | bestehend (Patch 3.2) |
| P03 | Haar-$L^2$ Firewall | bestehend |
