...
> **Aktuelle Research-Basis:** `main@2102b538c220cd809ad876c425df4f30304eb997`; aktive Härtung auf `research/r43-gcac-hardening`, mathematischer R43-Head `019e4e55e15b4e1df77c15d5b76a992c374d0510`. Quellen-/Analytikhärtung des exakten Kandidaten: publizierte Bessonov–Denisov-Fassung eindeutig fixiert, `L_H(Q)` als generalisierter Inversradius, sesquilineare Riesz-Holomorphie via reeller Formkomplexifizierung. **GC-M1_scalar** bleibt candidate-GREEN; **GC-AC** bleibt candidate-closed ohne neue `independent GREEN`-Buchung. Neu für den letzten Strong-Terminal-Rest: `b_U` ist reell, `Strong Terminal ⇔ [b_U Cauchy] & [b_U^2→1]`, und `B-TIGHT` ist als exaktes uniformes Tail-Kriterium gebucht. R38–R42 bleiben FROZEN — independently verified AI-GREEN ohne kanonische `✓[M]`-Promotion.  
...
| **R43** | OPEN — mixed reviewed/candidate status | Frühere reviewte Schichten bleiben GREEN; aktueller mathematischer Head `019e4e55e1...`: Literaturfassung/Typ-Radius gehärtet, Riesz-Analytizität über reelle Formkomplexifizierung repariert; `GC-M1_scalar` candidate-GREEN und §3K `GC-AC` candidate-closed, unabhängiger Review des neuen Heads ausstehend; unter diesem Kandidaten ist `b_U∈ℝ`, Strong Terminal iff `b_U` Cauchy und `b_U^2→1`; `B-TIGHT` = iff-uniforme Tail-Kontrolle des schwach verschwindenden tangentialen Restes | `audits/P11_REFEREE_E2E_R43_SINGLE_NORMAL_C6_GATE_2026-09-02.md` |
