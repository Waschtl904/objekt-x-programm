...
| **R43** | OPEN — mixed reviewed/candidate status | Mathematischer Head `ade2cd34e7...`: GC-AC-Härtung inkl. explizitem Singularträger-Schritt; `GC-M1_scalar` candidate-GREEN, `GC-AC` candidate-closed. External destructive GREEN (Perplexity, cross-model nonblind) gilt exakt für den früheren Head `c7c6f04c...` und die sieben GC-AC-Prüfpunkte, nicht für spätere Mathematik. `b_U∈ℝ`; Strong Terminal iff `b_U` Cauchy und `b_U^2→1`; `B-TIGHT` = iff-uniforme Tail-Kontrolle; unter B-TIGHT: Strong Terminal iff `b_Tb_U→1`; `B-SIGN/B-ORIENT` offen; kein Clark-/Randparameter-Import ohne Identifikation | `audits/P11_REFEREE_E2E_R43_SINGLE_NORMAL_C6_GATE_2026-09-02.md`; Review-Provenienz: `audits/P11_R43_EXTERNAL_REVIEW_C7C6F04C_RECONCILIATION_2026-09-03.md` |
