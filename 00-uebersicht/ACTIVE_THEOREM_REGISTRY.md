...
> **Aktuelle Research-Basis:** `main@2102b538c220cd809ad876c425df4f30304eb997`; aktive Härtung auf `research/r43-gcac-hardening`, mathematischer R43-Head `c4d0f03089659533fb06bf9a2822060c64e2d9e1`, kanonischer R43-Blob `983b42949d6a4a1806c0b333727cb49000b99972`. **GC-M1_scalar** bleibt candidate-GREEN; **GC-AC** bleibt candidate-closed. Perplexity bleibt ausschließlich auf exakt `c7c6f04cd601ea868cb536327504f6c90b3f0807` / Blob `74a91c71b8b08f60d448811c57ceeca6f6113c87` als **external destructive GREEN (cross-model nonblind)** gebucht; keine formale `independent GREEN (cross-model)`. Primäres offenes Gate ist **B-FLAGTIGHT**, nicht die stärkere Jet-Zahlenergie. Der moving-range Kozykel `T_{U→V}=W_VW_U^*` ist algebraisch exakt, aber sein Off-Flag-Block enthält bereits bei `U=V` einen statischen Range/Flag-Winkel. Die bevorzugte inkrementelle Formulierung ist daher **B-FLAGDYN** auf dem festen Quellraum mit `Q_{m,U}=W_U^*P_mW_U`, `q_m(U)=<ε_R,Q_{m,U}ε_R>` und echter Terminaldynamik `Q_{m,V}-Q_{m,U}`, exakt entwickelt über die C2-Horizon-Gauges `C_X^{U→V}`. Nach B-FLAGTIGHT bleibt **B-SIGN/B-ORIENT** OPEN; unter B-TIGHT gilt scharf `Strong Terminal ⇔ liminf L>-1`. Kein Clark-/Darboux-/Generator-Import ohne separaten Satz. R38–R42 bleiben FROZEN — independently verified AI-GREEN ohne kanonische `✓[M]`-Promotion.  
...
| **R43** | OPEN — mixed reviewed/candidate status | Mathematischer Head `c4d0f030896...`: GC-AC candidate-closed; exaktes Primärgate **B-FLAGTIGHT** ist `lim_m limsup_U ||P_{H_S^[m]}h_U||=0`. Die Jet-Zahl-/Flaggenenergie bleibt nur ein stärkeres hinreichendes Werkzeug. Der normalized constrained-Gamma Jet-Gram-Test kontraindiziert einen globalen uniformen Higher-Jet-Riesz-Unterrand als Hauptweg, ohne theorem-level No-Go. Der partielle Kozykel `T_{U→V}=W_VW_U^*` bleibt exaktes Bookkeeping, aber `T_{U→U}=P_{Ran W_U}`; daher misst sein Off-Flag-Block auch einen statischen Range/Flag-Winkel und ist kein reines Terminalinkrement. Neuer bevorzugter Quantitätsknoten **B-FLAGDYN**: `Q_{m,U}=W_U^*P_mW_U`, `q_m(U)=<ε_R,Q_{m,U}ε_R>`, echte Änderung `Q_{m,V}-Q_{m,U}`, mit exakter C2-Horizon-Gauge-Entwicklung und O1-Modulus/Range/Polarphase als möglichem Input. Kommutator/Gronwall bleibt ohne Terminalgenerator gefirewallt. Nach B-TIGHT gilt die scharfe Restbedingung `Strong Terminal ⇔ liminf_{T,U→∞}L_{R,S}^{T,U}>-1`; lokale Inkrementkriterien benötigen Kettenzusammenhang. External destructive GREEN (Perplexity, cross-model nonblind) bleibt strikt auf `c7c6f04c...` beschränkt. | `audits/P11_REFEREE_E2E_R43_SINGLE_NORMAL_C6_GATE_2026-09-02.md`; `audits/P11_R43_FLAG_TIGHTNESS_COCYCLE_AND_SIGN_HARDENING_2026-09-03.md`; `audits/P11_R43_ORBIT_ADAPTED_FLAG_LEAKAGE_TELESCOPING_2026-09-03.md`; Numerik: `audits/P11_R43_JET_GRAM_CONDITIONING_DIAGNOSTIC_2026-09-03.md` |
