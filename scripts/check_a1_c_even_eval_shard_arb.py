# -*- coding: utf-8 -*-
"""Sharded rigorous node-evaluation certificate for the frozen A1 C-even gate.

Each shard checks a disjoint round-robin subset of the 3878 Gauss panels.
At every one of its Gauss nodes it proves the even Legendre-Fourier proposal
vector has l2 error < 1e-43, using the direct-turning-anchor engine plus the
exact even Bessel energy identity.  It also accumulates the scalar alpha-ball
contribution to the nodewise perturbation lemma.

All 32 shards together imply the scalar-alpha evaluation contribution is
<1e-39.  A separate global-budget checker combines this with the analytic
uniform vector-error contribution to certify epsilon_eval <4e-39.
"""
...
if SHARD_COUNT != 32:
    raise RuntimeError("frozen C-even shard count is 32")
if not (0 <= SHARD_INDEX < SHARD_COUNT):
    raise RuntimeError("invalid shard index")


def sharp_even_head(z: arb):
    """Return rigorous sharp even j_n intervals through the direct high anchor."""
...
def main() -> None:
    ...
    for panel in range(SHARD_INDEX, PANEL_COUNT, SHARD_COUNT):
        ...
        for node_index in range(GAUSS_N):
            ...
            head, _, _ = sharp_even_head(x)
            eb = vector_error_bound(x, head)
            if not (eb < E_B_TARGET):
                raise RuntimeError(
                    f"node e_b target failed: shard={SHARD_INDEX}, panel={panel}, "
            ...
            ealpha = A(alpha.rad())
            scalar_sum += ealpha * SCALAR_FACTOR
            node_count += 1
    ...
    print("A1 C-even evaluation shard certificate")
    ...
    print("CERTIFIED: every node in this shard has C-even vector error e_b < 1e-43")
    print("CERTIFIED: scalar-alpha perturbation contribution is below 1e-39/32")
    print("FIREWALL: shard success is not finite matrix positivity")
