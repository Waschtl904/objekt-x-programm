...
/*
 * Direct rigorous SPD gate for the frozen A1 C-odd exact integer matrix.
 *
 * This checker reads the already-built binary artifact, reconstructs the
 * exact symmetric fmpz matrix, independently recomputes the canonical
 * row-major SHA-256 used by the Python builder, and then asks Arb for a
 * rigorous LDL^T certificate.
 *
 * arb_mat_ldl() returns nonzero only if positive definiteness is CERTAIN.
 * A zero return is UNDECIDED here, never a negative theorem.
 */
...
#define DIM 1075
#define MAGIC "A1CODD01"
#define MAGIC_LEN 8
#define MAX_MAG_BYTES 65535

static const char *EXPECTED_SHA256 =
    "6cff5e4710f0f23c4b43503e5b398525898e0a034e61c25791526ad94f17e295";
...
static void load_artifact(const char *path, fmpz_mat_t A)
{
    ...
    if (fread(magic, 1, MAGIC_LEN, f) != MAGIC_LEN ||
        ...
        die("C-odd matrix artifact magic mismatch");

    if (read_u32_be(f) != DIM)
        die("C-odd matrix artifact dimension mismatch");
    ...
    if (fgetc(f) != EOF)
        die("C-odd matrix artifact has trailing bytes");
    if (ferror(f))
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) {
        fprintf(stderr, "usage: %s a1_c_odd_exact_matrix.bin\n", argv[0]);
        return 2;
    }
    path = argv[1];
    ...
    if (!fmpz_mat_equal(Z, T))
        die("decoded C-odd matrix is not symmetric");
    printf("CERTIFIED: decoded C-odd matrix is exactly symmetric 1075x1075\n");
    ...
    if (strcmp(digest, EXPECTED_SHA256) != 0)
        die("canonical C-odd matrix SHA-256 mismatch");
    printf("CERTIFIED: C checker reconstructed the frozen exact C-odd matrix fingerprint\n");
    fflush(stdout);

    for (k = 0; k < ladder_len; k++) {
        if (certify_spd_ldl(Z, ladder[k])) {
            printf("CERTIFIED_PRECISION=%ld\n", (long)ladder[k]);
            printf("CERTIFIED: exact hash-fixed 1075x1075 C-odd integer matrix is positive definite by Arb LDL\n");
            printf("CERTIFIED: A_o >= 1e-35 I_1075 after the frozen finite-model ledger\n");
            fmpz_mat_clear(T);
            fmpz_mat_clear(Z);
            flint_cleanup();
            return 0;
    ...
    printf("UNDECIDED: exact hash-fixed C-odd matrix loaded correctly, but Arb LDL did not certify positivity on the precision ladder\n");
    fmpz_mat_clear(T);
    fmpz_mat_clear(Z);
    flint_cleanup();
    return 3;
