...
> **Aktuelle Research-Basis:** `main@2102b538c220cd809ad876c425df4f30304eb997` enthält die konsolidierte R37–R43-Linie. Aktive Härtung: Branch `research/r43-gcac-hardening`; aktueller **mathematischer R43-Head** `366c5e3dcdd2b2f16bf8b2663c0284a4db9538f5` (R43-Mathematikblob `c843336d43dd13ca31e63f81550d545033a89d7d`), nachfolgend nur Review-Provenienz-/Ledger-/Navigationssync. Der user-supplied Perplexity-Review hält die sieben GC-AC-Härtungspunkte auf dem exakten früheren Head `c7c6f04cd601ea868cb536327504f6c90b3f0807` / Blob `74a91c71b8b08f60d448811c57ceeca6f6113c87` destruktiv GREEN; weil der Review ausdrücklich Prüfvektoren aus einem vorigen Turn wiederverwendet, wird er als **external destructive GREEN (cross-model nonblind)** und nicht als formales `independent GREEN (cross-model)` gebucht. Die aktuelle Mathematik härtet zusätzlich den Singularträger-Schritt R43.10cv0–cv2 und reduziert nach B-TIGHT den letzten Sign-Gate auf `b_T b_U→1`: `b_U∈[-1,1]`, `Strong Terminal ⇔ [b_U Cauchy] & [b_U^2→1]`; unter B-TIGHT äquivalent `Strong Terminal ⇔ b_Tb_U→1`. **B-TIGHT** bleibt offen und ist exakt als uniforme Kompaktheit des Normalrestes in der kanonischen C6a-ONB formuliert. Ein reproduzierbarer constrained-Gamma-Galerkin-Test der **normalisierten** Higher-Jet-Rieszfamilie zeigt stabile geometrische Konditionierungsdegeneration (`m=4,8,12,16`: `σ_min≈2.8e-3,2.4e-6,2.3e-9,2.2e-12` bei `S=1`); daher ist ein globaler uniformer Higher-Jet-Riesz-Unterrand numerisch stark kontraindiziert und nicht mehr Primärroute. Bevorzugt ist jetzt eine **direkte kompakt-resolvente Orbitenergie** für `h_U`. Ein Kommutator/Gronwall-Weg ist ohne neuen Terminalgenerator nur formal; R40/R41 liefern keine `dW_U/dU`-Schranke. Bevorzugt ist die intrinsische Flaggenenergie `Σ_{m≥1}||P_{H_S^[m]}h_U||²`, exakt gleich `⟨N_Sh_U,h_U⟩`; sie formuliert die Kompaktheitsroute ausschließlich über die kanonischen C6a-Jetkerne. Für **B-SIGN/B-ORIENT** ist statt Darboux primär ein lokaler strikter Inkrementabstand `<2` oder positive Zwei-Terminal-Korrelation vorgesehen. Ein Aleksandrov–Clark-/kanonischer Randparameter wird für `b_U` ausdrücklich **nicht** importiert, solange keine Identifikationsformel bewiesen ist. R43 bleibt OPEN; kein Freeze, kein `✓[M]`, keine Strong-Terminal-/Object-X-/RH-Promotion.  
